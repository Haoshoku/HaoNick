...
public class HaoReloadHandler {
    ...
    public void executeAsyncPlayerPreLoginEvent( UUID uuid, String name ) {
        if ( uuid != null && name != null ) {
            HaoUser user = HaoUserHandler.getUserByUUID( uuid );
    ...
    public void executeOnLoginEvent( Player player ) {
        ...
        for ( Player players : Bukkit.getOnlinePlayers() ) {
            ...
            if ( NickAPI.getUniqueId( players ).equals( player.getUniqueId() ) || NickAPI.getName( players ).equalsIgnoreCase( player.getName() )
                ...
                if ( NickAPI.getOriginalName( players ).equalsIgnoreCase( user.getNickedTag() ) )
                    resetPlayer = player;
                else
                    resetPlayer = players;

                NickAPI.resetUniqueId( resetPlayer );
                NickAPI.resetNick( resetPlayer );
                NickAPI.resetGameProfileName( resetPlayer );
                NickAPI.resetSkin( resetPlayer );
                NickAPI.refreshPlayerSync( resetPlayer );
                MsgUtils.sendMessage( resetPlayer, HaoNick.getPlugin().getConfigManager().getMessagesConfig().getMessage( "messages.join_reset", resetPlayer ) );
            }
        }
    }
